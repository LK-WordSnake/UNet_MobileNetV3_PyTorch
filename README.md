...
class Double_Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), inplace=True):  # detection layer
        ...
        # Init decouple head
        self.cls_convs = nn.ModuleList(nn.Conv2d(256, 256, 1) for _ in range(len(ch)))
        self.reg_convs = nn.ModuleList(nn.Conv2d(256, 256, 1) for _ in range(len(ch)))
        self.cls_preds = nn.ModuleList(nn.Conv2d(256, self.nc * self.na, 1) for _ in range(len(ch)))
        self.reg_preds = nn.ModuleList(nn.Conv2d(256, 4 * self.na, 1) for _ in range(len(ch)))
        self.obj_preds = nn.ModuleList(nn.Conv2d(256, 1 * self.na, 1) for _ in range(len(ch)))
        self.stems = nn.ModuleList(nn.Conv2d(x, 256, 1) for x in ch)
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            obj_pred = self.obj_preds[i](reg_feat)

            x[i] = torch.cat([reg_pred, obj_pred, cls_pred], dim=1)
